...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.cornerVisited=[False,False,False,False] # initialized corner visit
    ...
    def getSuccessors(self, state):
        ...
        successors = []
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:        
            x,y = state[0]  #the curretn state
            cornerVisited = state[1]
            dx,dy = Actions.directionToVector(action)
            x_new,y_new = int(x + dx),int(y + dy)   # new position
            cornerVisited_new = cornerVisited[:]
            isWall = self.walls[x_new][y_new]
            if not isWall:
                cornerIndex = 0 # index value of new corner
                for corner in self.corners: # check if any of the corner is visited
                    if (x_new,y_new) == corner:
                        break
                    cornerIndex += 1

                if cornerIndex < 4: 
                    cornerVisited_new[cornerIndex] = True # put visited
                state_new = ((x_new,y_new),cornerVisited_new)
                cost = self.costFn(x_new,y_new)
                successors.append((state_new,action,cost)) # add a successor
